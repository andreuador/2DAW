...
class Empleado {
  ...
  public $email;                 // Dirección de correo electrónico del empleado.
    ...
    public function __construct(int $id, string $nombre, string $apellido, string $tipo, string $email, string $contrasena, string $confirmarCon) {
        $this->id = obtener($id);
        $this->nombre = obtener($nombre);
        $this->apellido = obtener($apellido);
        $this->tipo = obtener($tipo);
        $this->email = obtener($email);
        $this->contrasena = obtener($contrasena);
        $this->confirmarCon = obtener($confirmarCon);
    }
